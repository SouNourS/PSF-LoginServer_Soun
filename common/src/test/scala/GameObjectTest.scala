...
class GameObjectTest extends Specification {
  "Game objects" in {
    ...
    "ExoSuit" should {
      "constructor (default)" in {
        val suit = new ExoSuit(ExoSuitType.Agile)
      ...
      "{object}.toString" in {
        val obj : ExoSuit = new ExoSuit(ExoSuitType.Agile)
        obj.name = "newSuit"
        obj.toString mustEqual "{newSuit}"
      }

      "ExoSuit.toString(object)" in {
        val obj : ExoSuit = ExoSuit(ExoSuitType.Agile)
        obj.name = "newSuit"
    ...
    "ExoSuitCatalog" should {
      "get standard exo-suit" in {
        val suit : ExoSuit = ExoSuitCatalog.get(ExoSuitType.Agile).get
