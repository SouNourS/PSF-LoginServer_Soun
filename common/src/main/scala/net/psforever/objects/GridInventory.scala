...
class GridInventory(w : Int, h : Int) extends Inventory(w, h) {
  /**
    ...
    */
  protected var contents : mutable.HashMap[Int,InventoryItem2] = new mutable.HashMap[Int,InventoryItem2]
  /**
  ...
  def addItem(item : Equipment, y : Int, x : Int) : (Boolean, Option[Equipment]) = {
    ...
    (overlap.size: @switch) match {
      case 0 =>
        contents += (item.guid -> InventoryItem2(item, y, x))
  ...
  def getItem(item : Equipment) : Option[Equipment] = {
    contents.foreach({
      case (key : Int, value : InventoryItem2) =>
  ...
  def removeItem(guid : Int) : Option[Equipment] = {
    val removopt : Option[InventoryItem2] = contents.remove(guid)
    if(removopt.isDefined) {
      val invItem : InventoryItem2 = removopt.get
      val obj = invItem.obj
  ...
  def removeItem(item : Equipment) : Option[Equipment] = {
    contents.foreach { case (key: Int, stowed: InventoryItem2) =>
  ...
  override def resize(w : Int, h : Int) : List[Equipment] = {
    ...
    if(contents.nonEmpty) {
      ...
      contents.values.foreach({ value : InventoryItem2 => dropped += value.obj })
      contents.clear
